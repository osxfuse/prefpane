...
@interface KSSilentUpdateActionTest : SenTestCase 
@end


static NSString *const kTicketStorePath = @"/tmp/KSSilentUpdateActionTest.ticketstore";

@implementation KSSilentUpdateActionTest
...
- (void)testCreation {
  KSSilentUpdateAction *action = [KSSilentUpdateAction actionWithEngine:nil];
  STAssertNil(action, nil);
  
  action = [[[KSSilentUpdateAction alloc] init] autorelease];
  STAssertNil(action, nil);
  
  action = [[[KSSilentUpdateAction alloc] initWithEngine:nil] autorelease];
  STAssertNil(action, nil);
  
  KSUpdateEngine *engine = [KSUpdateEngine engineWithDelegate:self];
  action = [KSSilentUpdateAction actionWithEngine:engine];
...
- (void)testPrompting {
  ...
  KSSilentUpdateAction *action = [KSSilentUpdateAction actionWithEngine:engine];
  STAssertNotNil(action, nil);
